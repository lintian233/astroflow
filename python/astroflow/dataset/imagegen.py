...
class MultiViewImageGenerator:
    ...
    def _time_span_with_pulse_fraction(
        ...
        """
        Choose t_sample so dedisperse_spec's fixed chunks place TOA near a target fraction.

        dedisperse_spec creates chunks [idx * t_sample, (idx + 1) * t_sample].
        ...
        Algorithm:
            For chunk idx containing toa_inject with target_fraction f:
                toa_inject = (idx + f) * t_sample
                t_sample = toa_inject / (idx + f)
        ...
        approx_idx = max(int(toa_inject / preferred_span), 0)
        candidates = []
        for idx in range(max(0, approx_idx - 10), approx_idx + 11):
            denom = idx + target_fraction
            if denom <= 0:
                continue
